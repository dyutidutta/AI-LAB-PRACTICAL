...
void astar(Node* start, int gi, int gj, int N,  vector<pair<int,int>>& water) {
    ...
    while (!pq.empty()) {
        ...
        list<Node*> neighbors = genmove(curr, gi, gj, N, water);
        for (Node* n : neighbors) {
            if (n->h == INFINITY) continue;
...
int main() {
    ...
    Node* start = new Node{si, sj, 0.0, start_h, NULL};

    astar(start, gi, gj, N, water);
    return 0;
}
